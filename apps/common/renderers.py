...
class ApiRenderer(BaseRenderer):
    ...
    def format_response(cls, http_status: int, payload: dict or str):
        ...
        if http_status in range(200, 300):
            ...
            logger.info(str({"error": response_body['error'], 'detailedError': payload}))
    ...
    def render(self, data, accepted_media_type=None, renderer_context=None):
        ...
        :return:
        """
        response_dict = ApiRenderer.format_response(http_status=renderer_context.get('response').status_code, payload=data)
